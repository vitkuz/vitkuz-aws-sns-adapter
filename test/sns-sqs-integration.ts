...
const STACK_NAME = 'vitkuz-testing-sns-sqs';
...
const getStackResources = async () => {
    ...
    const lambda = response.StackResources.find(
        (r) =>
            ...
            r.LogicalResourceId?.includes('SqsProcessor'),
    );
...
const run = async () => {
    try {
        ...
        // We look for the requestId because that proves the logger context picked it up
